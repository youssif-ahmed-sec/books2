...
import { useInventoryStore, InventoryBalance } from '@/store/useInventoryStore';
import TransactionSlideOver from '@/components/TransactionSlideOver';
import { Plus, PackageSearch, RefreshCw, Upload, Download } from 'lucide-react';

export default function InventoryPage() {
  const { balances, stats, recentTransactions, isLoading, error, fetchBalances, fetchStats, fetchRecentTransactions, openTransactionModal, submitTransaction } = useInventoryStore();
  ...
  }, [fetchBalances, fetchStats, fetchRecentTransactions]);

  const handleExport = () => {
    if (balances.length === 0) return;
    
    // Create CSV content
    const headers = ['معرف المنتج', 'اسم المنتج', 'المستودع', 'الكمية الحالية', 'تاريخ التحديث'];
    const rows = balances.map(b => [
      b.product_id,
      b.product_name || '',
      b.warehouse_name || b.warehouse_id,
      b.current_stock.toString(),
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      // Basic validation: Check if we have at least headers and one row
      if (rows.length < 2) {
      ...
      let importedCount = 0;
      // Skip headers, process each row
      for (let i = 1; i < rows.length; i++) {
        const cols = rows[i];
        if (cols.length < 4) continue; // Skip invalid rows
        
        const productId = cols[0]?.trim();
        const warehouseId = cols[2]?.trim() || 'default';
        // Check if there is an existing balance to compare or just add adjustment
        const newStock = parseFloat(cols[3]);
        
        if (productId && !isNaN(newStock)) {
          // Send an adjustment transaction
          const existing = balances.find(b => b.product_id === productId);
          const current = existing?.current_stock || 0;
          ...
          if (diff !== 0) {
            await submitTransaction({
              ...
              notes: 'تسوية مستوردة من ملف CSV'
            });
            importedCount++;
          }
        }
      ...
      alert(`تم استيراد/تحديث ${importedCount} صنف بنجاح`);
  ...
  return (
    <>
      <div className="space-y-10 rtl fade-in-up">
        {/* Header Actions */}
        <div className="flex items-end justify-between">
          <div>
            <h3 className="text-4xl font-bold text-on-surface tracking-tight">قائمة المنتجات</h3>
            <p className="text-on-surface-variant/70 mt-2 text-lg">إجمالي المسجل: <span className="text-primary font-bold">{balances.length}</span> صنف متاح</p>
          </div>
          <div className="flex items-center gap-4">
            ...
            <button 
              ...
              className="flex items-center gap-3 px-6 py-3.5 glass text-on-surface rounded-full hover:bg-white/10 transition-all font-bold text-sm"
            >
          ...
          <div className="col-span-12 lg:col-span-8 space-y-8">
            ...
            <div className="glass rounded-2xl overflow-hidden hi-fi-shadow">
                ...
                <table className="w-full text-right border-collapse">
                  <thead>
                    <tr className="bg-white/5">
                      ...
                      <th className="px-8 py-5 text-[11px] font-bold text-on-surface-variant/60 uppercase">الرصيد الحالي</th>
                    ...
                    {isLoading && balances.length === 0 ? (
                      Array.from({ length: 3 }).map((_, i) => (
                        <tr key={i} className="animate-pulse">
                          <td className="px-8 py-6"><div className="h-10 bg-surface-bright rounded-2xl w-32"></div></td>
                          ...
                          <td className="px-8 py-6"><div className="h-6 bg-surface-bright rounded-full w-16"></div></td>
                          <td className="px-8 py-6"><div className="h-8 bg-surface-bright rounded-full w-32"></div></td>
                          <td className="px-8 py-6"><div className="h-8 bg-surface-bright rounded-full w-24 mx-auto"></div></td>
                        </tr>
                      ))
                    ) : balances.length > 0 ? (
                      balances.map((balance) => {
                        const stockPercentage = Math.min(100, (balance.current_stock / 100) * 100);
                        const isCritical = balance.current_stock < 20;
                        return (
                          <tr key={balance.id} className="hover:bg-white/5 transition-all">
                            <td className="px-8 py-6">
                              <div className="flex items-center gap-5">
                                ...
                                <div>
                                  <p className="font-bold text-on-surface">{balance.product_name || balance.product_id}</p>
                                  <p className="text-[10px] text-on-surface-variant/60">تفاصيل المنتج</p>
                                  <span className="text-[9px] bg-primary/10 text-primary px-2 py-0.5 rounded-full mt-1 inline-block border border-primary/20">عام</span>
                                </div>
                              </div>
                            </td>
                            <td className="px-8 py-6">
                              <div className="flex flex-col gap-1">
                                <span className="font-mono text-primary font-bold text-xs">#SKU-{balance.product_id.split('-')[0]}</span>
                                <span className="font-mono text-[9px] opacity-40">N/A</span>
                              </div>
                            </td>
                            ...
                            <td className="px-8 py-6">
                              <div className="text-[10px] leading-relaxed">
                                <p><span className="opacity-50">تجزئة:</span> <span className="font-bold text-primary text-sm">0.00</span></p>
                                <p><span className="opacity-50">جملة:</span> <span className="font-bold">0.00</span></p>
                              </div>
                            </td>
                            <td className="px-8 py-6">
                              <div className="w-40">
                                <div className="flex justify-between text-[10px] mb-2 font-bold">
                                  <span className={isCritical ? "text-error" : ""}>{balance.current_stock.toLocaleString()} قطعة</span>
                                  <span className={isCritical ? "text-error" : "text-primary"}>{Math.round(stockPercentage)}%</span>
                                </div>
                                <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                                  <div className={`h-full rounded-full shadow-[0_0_10px_rgba(255,107,0,0.3)] ${isCritical ? 'bg-error shadow-[0_0_10px_rgba(255,180,171,0.3)]' : 'bg-primary'}`} style={{ width: `${stockPercentage}%` }}></div>
                                </div>
                              </div>
                            </td>
                            <td className="px-8 py-6">
                              <div className="flex items-center justify-center gap-3">
                                ...
                                <button onClick={() => openTransactionModal(balance.product_id)} className="w-9 h-9 rounded-full glass flex items-center justify-center text-on-surface-variant/60 hover:text-primary hover:border-primary/30 transition-all"><span className="material-symbols-outlined text-[18px]">edit</span></button>
                                <button className="w-9 h-9 rounded-full glass flex items-center justify-center text-on-surface-variant/60 hover:text-primary hover:border-primary/30 transition-all"><span className="material-symbols-outlined text-[18px]">history</span></button>
                              </div>
                            </td>
                          </tr>
                        );
                      ...
                      <tr>
                        <td colSpan={6} className="px-8 py-12 text-center">
                          ...
                          <p className="text-on-surface-variant">لا توجد بيانات مخزون</p>
                          <button
                            onClick={() => openTransactionModal()}
                            className="mt-6 px-6 py-2 rounded-full border border-white/10 text-primary font-medium text-sm hover:bg-white/5 transition-colors"
                          >
                            إنشاء أول حركة مخزون
                          </button>
                        </td>
                      </tr>
                    )}
              ...
              <div className="px-8 py-5 border-t border-white/5 flex items-center justify-between">
                <span className="text-xs font-medium text-on-surface-variant/60">عرض 1-10 من أصل <span className="text-on-surface">{balances.length}</span></span>
                ...
                {balances.filter(b => b.current_stock < 20).slice(0, 3).map(balance => (
                  <div key={balance.id} className="flex items-center justify-between p-4 rounded-2xl bg-white/5 border border-white/5 group hover:bg-white/10 transition-all cursor-pointer">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-sm font-bold">{balance.product_name}</p>
